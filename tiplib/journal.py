...
class Journal:
    def __init__(self, wd_id=None, issn=None):
        ...
        if wd_id is not None:
            ...
            print(
                "Please insert ISSN or Wikidata-ID to retrieve information on journals."
            )
    ...
    def _get_country_of_origin(self):
        ...
        q_results = self._get_wd_data(query)
        self.country_of_origin = q_results["results"]["bindings"][0]["countryLabel"][
            "value"
        ]
    ...
    def _get_review_score(self):
        ...
        if (len(q_results["results"]["bindings"])) > 0:
            self.review_score = q_results["results"]["bindings"][0]["ReviewScoreLabel"][
                "value"
            ]
    ...
    def _get_main_subject(self):
        ...
        q_results = self._get_wd_data(query)
        self.main_subject = q_results["results"]["bindings"][0]["mainSubjectLabel"][
            "value"
        ]
